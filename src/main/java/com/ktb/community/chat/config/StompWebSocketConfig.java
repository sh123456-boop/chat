...
public class StompWebSocketConfig implements WebSocketMessageBrokerConfigurer {
    ...
    public void registerStompEndpoints(StompEndpointRegistry registry) {
        registry.addEndpoint("/v1/chat/connect")
                .setAllowedOrigins(front,"http://localhost:8080", "http://127.0.0.1:8080")
...
//        /publish로 시작하는 url패턴으로 메시지가 발행되면 @Controller 객체의 @MessaMapping메서드로 라우팅
        registry.setApplicationDestinationPrefixes("/v1/chat/publish");

//        /topic/1형태로 메시지를 수신(subscribe)해야 함을 설정
        registry.enableSimpleBroker("/v1/chat/topic");
    }
